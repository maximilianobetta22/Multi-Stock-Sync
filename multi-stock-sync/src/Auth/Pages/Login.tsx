...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rolesPermitidos = [1, 2, 3, 4, 5, 6]; // Admin, Logística, Finanzas, RRHH, vendedor, plugin
      if (!user.role_id || !rolesPermitidos.includes(user.role_id)) {
        setError(
          "Tu cuenta aún no tiene permisos asignados. Contacta al administrador."
        );
        setLoading(false);
        return;
      }
      ...
      setError(
        (err as any).response?.data?.message || "Credenciales inválidas"
      );
      console.log(err);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      {!loading && (
        ...
        >
          {/* ...elementos de fondo animados y partículas (omitido para brevedad)... */}
                ...
                <div style={{ padding: "25px 20px" }}>
                  ...
                  <form onSubmit={handleSubmit}>
                    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
                      ...
                      <Divider style={{ margin: "15px 0", fontSize: "11px" }}>
                        ¿No tienes cuenta?
                      </Divider>
