...
import { SaleService, type SaleHistoryItem } from "../Services/saleService"
...
import type { Products } from "../Types/ventaTypes"
...
<<<<<<< HEAD
  ...
  const columnasItems = useMemo(
    () => [
      {
        title: <Text strong>Producto</Text>,
        dataIndex: "product_name",
        key: "product_name",
      },
      {
        title: <Text strong>Cantidad</Text>,
        dataIndex: "quantity",
        key: "quantity",
      },
      {
  ...
  const productosVenta = useMemo((): Products[] => {
    if (selectedSaleDetails && selectedSaleDetails.products) {
      ...
      if (Array.isArray(selectedSaleDetails.products)) {
        return selectedSaleDetails.products
      }
      // Si products viene como string JSON (compatibilidad con versiones anteriores)
      if (typeof selectedSaleDetails.products === "string") {
        try {
          const parsedItems = JSON.parse(selectedSaleDetails.products)
          if (Array.isArray(parsedItems)) {
            return parsedItems.map((item: any) => ({
              product_name: String(item.nombre || item.product_name || ""),
              product_id: item.id || item.product_id || "",
              quantity: Number(item.cantidad || item.quantity || 0),
              price_unit: Number(item.precioUnitario || item.price_unit || 0),
            ...
            <Row gutter={[8, 8]} align="middle">
              <Col span={16}>
                <Form.Item
                  label={
                    <Text strong>
                      <FileSearchOutlined /> Buscar por Folio o Cliente
                    </Text>
                  }
                ...
                <>
                  ...
                  <Row gutter={[16, 16]}>
                    <Col xs={24} md={12}>
                      ...
                      <br />
                      <Text strong>
                        <ShopOutlined /> Bodega ID:
                      </Text>{" "}
                      {selectedSaleDetails.warehouse_id}
                      <br />
                      <Text strong>
                      ...
                      {selectedSaleDetails.observation || "Sin observaciones"}
                    </Col>
                    <Col xs={24} md={12}>
                      <div style={{ textAlign: "right" }}>
                        <Title level={5} style={{ margin: 0 }}>
                          Subtotal: ${selectedSaleDetails.price_subtotal?.toFixed(2).replace(/\.00$/, "") || "0"}
                        </Title>
                        <Title level={4} style={{ margin: "8px 0 0 0", color: "#1890ff" }}>
                          Total: ${selectedSaleDetails.price_final?.toFixed(2).replace(/\.00$/, "") || "0"}
                        </Title>
                      </div>
                    </Col>
                  </Row>
                  ...
                  <Table
                    ...
                    pagination={false}
                    rowKey={(record, index) => String(record.product_id || index)}
                    size="middle"
