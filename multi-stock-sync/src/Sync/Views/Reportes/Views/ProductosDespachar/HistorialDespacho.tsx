...
const HistorialDespacho: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `historial_producto_${productId}.xlsx`);
  };

  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text(`Historial de Producto ${productId}`, 14, 10);
    doc.autoTable({
  ...
  const handleDownload = () => {
    ...
    doc.save(`historial_producto_${productId}.pdf`);
    setShowModal(false);
  };
  ...
  return (
    <Container className={styles.container}>
      ...
      <Form className={styles.filtroContainer} onSubmit={(e) => { e.preventDefault(); buscarHistorial(); }}>
        <Form.Control
          type="text"
          placeholder="Ingrese el Product ID"
          value={productId}
          onChange={(e) => setProductId(e.target.value)}
        />
        <Button variant="dark" onClick={buscarHistorial}>Buscar</Button>
      </Form>

      {loading && <Spinner animation="border" className="d-block mx-auto" />}
      {error && <Alert variant="danger">{error}</Alert>}
      {notFound && <Alert variant="warning">No se encontraron registros para este producto.</Alert>}
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg">
        ...
        <Modal.Body>{pdfUrl && <iframe src={pdfUrl} width="100%" height="500px" title="PDF Preview" />}</Modal.Body>
        <Modal.Footer>
          <Button variant="dark" onClick={handleDownload}>Descargar PDF</Button>
        </Modal.Footer>
      </Modal>
    </Container>
  );
