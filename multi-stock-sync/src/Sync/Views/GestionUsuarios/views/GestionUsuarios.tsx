    ...
    try {
      ...
      for (const [userId, nuevoRolId] of Object.entries(cambiosPendientes)) {
        try {
          ...
          message.error(`Error al actualizar usuario ${userId}`);
        }
      }
    ...
    },
    {
      ...
      render: (_: any, record: Usuario) => (
        <Select
          ...
          disabled={saving || (roleId === 4 && record.role_id === 1)}
          size="middle"
        >
          {rolesDisponibles.map(rol => (
            <Option key={rol.id} value={rol.id} disabled={roleId === 4 && rol.id === 1}>
              <Space>
            ...
            <Col>
              <Space align="center">
                ...
                {Object.keys(cambiosPendientes).length > 0 && (
                  <Badge count={Object.keys(cambiosPendientes).length} style={{ backgroundColor: "#fa8c16" }}>
